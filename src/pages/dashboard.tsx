...
import { getUserHistories } from 'apiSdk/user-histories';
import { UserHistoryInterface } from 'interfaces/user-history';
import { getExperienceReviews } from 'apiSdk/experience-reviews';
import { ExperienceReviewsInterface } from 'interfaces/experience-reviews';
import { getTripPlans } from 'apiSdk/trip-plans';
import { TripPlanInterface } from 'interfaces/trip-plan';

import { HostelAmenitiesListPage as ShowCaseList } from 'pages/hostel-amenities';

function DashboardPage() {
  const { session } = useSession();

  const userHistoryStat = useSWR<PaginatedInterface<UserHistoryInterface>>(
    () => `/user-histories`,
    () => getUserHistories({ limit: 1, offset: 0 }),
  );

  const experienceReviewsStat = useSWR<PaginatedInterface<ExperienceReviewsInterface>>(
    () => `/experience-reviews`,
    () => getExperienceReviews({ limit: 1, offset: 0 }),
  );

  const tripPlanStat = useSWR<PaginatedInterface<TripPlanInterface>>(
    () => `/trip-plans`,
    () => getTripPlans({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'User Histories',
      result: userHistoryStat?.data?.totalCount,
      isLoading: userHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Experience Reviews',
      result: experienceReviewsStat?.data?.totalCount,
      isLoading: experienceReviewsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trip Plans',
      result: tripPlanStat?.data?.totalCount,
      isLoading: tripPlanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Hostel Amenities
                </Text>
                <NextLink href={`hostel-amenities`} passHref legacyBehavior>
                  <Button
